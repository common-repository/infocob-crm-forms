...
class FieldTel extends Field
{
	public function display() {
        ...
        <label class="<?php echo $this->getCssClasses(); ?>">
			...
			<?php } ?>
            <input type="tel" name="<?php echo esc_attr($this->nom); ?>" placeholder="<?php echo $libelle; ?>" value="" <?php if($this->required){ ?>required<?php } ?> />
        </label>
		<?php
	}
	...
	public function get() {
		...
		return '
            <label class="' . $cssClasses . '">
                ' . $is_display_libelle . '
                <input type="tel" name="' . $nom . '" placeholder="' . $libelle . '" value="' . $valeur . '" ' . $input_required . ' />
            </label>
			';
	}
